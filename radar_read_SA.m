...
tic	 %启动秒表计时
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%基数据文件路径定义%%%%%%%%%%%%%%%%%%%%%%
filename='D:\test\cape\Z_RADR_I_Z9250_20160705075000_O_DOR_SA_CAP.bin';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fid=fopen(filename);
fseek(fid,0,'eof');
...
%%%%%%%%%%%%%%%%数据基础处理与作图%%%%%%%%%%%%%%%%
global VCP
VCP=Data(1).VCP; %获取VCP，以便画出图像后自动显示扫描模式
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ii=1:N
...
for nn=1:Cutnumer
    ...
    while DataNumber==Data(jj).DataNumber
        ...
        cut(nn).RefData(:,DataNumber)=Data(jj).CorrectRel(1:200);%TransRel; %【反射率数据库数*径向个数】反射率dBz
...
%%%%%%%%%%%%日期计算%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Date=jd2cal(Data(1).RadialCollectionDate+2440587);  
% global Year           %设为全局变量以便在作图函数中引用，显示在图像中
% Year=Date(1);
% global Month 
% Month=Date(2);
% global Day
% Day=Date(3);
% %%%%%%%%%%时间计算%%%%%%%%%%%%
% Seconds=Data(ii).RadialCollectionTime/1000;
% global hour 
% hour =fix( Seconds/3600);
% global minute
% minute= fix((Seconds-hour*3600)/60 );
% global seconds
% seconds =fix(Seconds -(60*hour+minute)*60) ;
%%%%%%%%%%%%%%%%%%%%%%%%画图部分%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for jj=1:Cutnumer
    R=1:200;     %R的长度为反射率数据库数   
%改变cut(jj).GateNumber值为常数，就可以改变图像显示范围（公里数）
    cut(jj).X=R'*cosd(cut(jj).Theta)*cosd(cut(jj).ElevationAngle);
    cut(jj).Y=R'*sind(cut(jj).Theta)*cosd(cut(jj).ElevationAngle);
end
%画图%%%%%%%%%%%%%
ii=1;  %需画图的层数，改变此值可以改变层数
a=cut(ii).X;
b=cut(ii).Y;
c=cut(ii).RefData;
a=a(1:150,:);
