...
laplace=fittype(@(a,b,x) a*exp(-abs(x)./b)-10,'independent','x');    %a*exp(-abs(x)./b)
...
p11 = predint(laplacefit,xi,0.95,'functional','off');


figure; plot(laplacefit,xi, yi, 'b.');  % 绘制数据点
hold on, plot(xi,p11,'m--')
...
figure
area(cdate,pop_fit+2*delta,'FaceColor','blue')
hold on
area(cdate,pop_fit-2*delta,'FaceColor','white')
plot(cdate,pop,'+',...
     cdate,pop_fit,'g-',...
